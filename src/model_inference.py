...
def model_fn(model_dir):
    ...
    try:
        ...
        if os.path.exists(artifacts_path):
            ...
            logger.warning("Artifacts file not found. Stateful features will use defaults.")
...
def transform_data(df, artifacts):
    ...
    required_cols = ['courier_lat', 'courier_lon', 'restaurant_lat', 'restaurant_lon', 'courier_location_timestamp']
    ...
    df["courier_location_timestamp"] = pd.to_datetime(df["courier_location_timestamp"], format="ISO8601")
    ...
    restaurants_counts_per_h3_index = artifacts.get("restaurants_counts_per_h3_index", {})
    ...
    if "h3_index" in encoders:
        ...
        df["h3_index"] = df["h3_index"].apply(lambda x: x if x in known_classes else le.classes_[0])
