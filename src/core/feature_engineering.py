...
def centroid_assignation(df, centroids):
    ...
    for i, obs in df.iterrows():
        all_errors = [calc_dist(centroid['lat'], centroid['lon'], obs['courier_lat'], obs['courier_lon']) for centroid in centroids_list]
...
def generate_features(df, restaurants_ids):
    ...
    df['order_created_timestamp'] = pd.to_datetime(df['order_created_timestamp'], format='ISO8601')
    df['h3_index'] = [h3.latlng_to_cell(lat, lon, resolution) for (lat, lon) in zip(df.courier_lat, df.courier_lon)]
